...
public class RpgCombatGame extends ApplicationAdapter {
	...
	public void create() {
		...
		final OrthographicCamera uiCamera = new OrthographicCamera();
	...
	private void initializeEntities() {
		...
		player = new Man(new Texture(Gdx.files.internal("man.png")));
		...
		enemy = new Man(new Texture(Gdx.files.internal("man2.png")));
	...
	private void initializeUserInterface() {
		...
		final UIInventoryButton inventory_button = new UIInventoryButton("Inventory", Gdx.graphics.getWidth() - 74, Gdx.graphics.getHeight() - 74, 64, 64);
		userInterface.addUIComponent(inventory_button);
		userInterface.addActionListener("Inventory", new UIAction() {

			@Override
			public void go(final AbstractUIComponent widget) {
	...
	public void render() {
		...
		final MovementComponent movementComponent = player.getEntityComponent(MovementComponent.class);
		...
		for (int y = 0; y < gameMap.getMapHeight(); y++) {
			for (int x = 0; x < gameMap.getMapWidth(); x++) {
				if (gameMap.getMapField(x, y).isCollision()) {
					...
					shapeRenderer.setColor(new Color(231 / 255f, 228 / 255f, 150 / 255f, 1));
				}
		...
		final CombatComponent combatComponent = player.getEntityComponent(CombatComponent.class);
		final Entity focusTarget = combatComponent.getFocusTarget();
		if (focusTarget != null) {
			final MovementComponent targetMovementComponent = focusTarget.getEntityComponent(MovementComponent.class);
			shapeRenderer.begin(ShapeType.Line);
			shapeRenderer.setColor(Color.GREEN);
			shapeRenderer.circle(targetMovementComponent.getPosition().x * 32, targetMovementComponent.getPosition().y * 32, 20);
			shapeRenderer.circle(targetMovementComponent.getPosition().x * 32, targetMovementComponent.getPosition().y * 32, 19);
			shapeRenderer.circle(targetMovementComponent.getPosition().x * 32, targetMovementComponent.getPosition().y * 32, 18);
		}
